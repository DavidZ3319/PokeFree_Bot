...
module.exports = {
    ...
    execute: async (client, message, args, prefix, guild, color, channel) => {
        ...
        if (!user || !user.pokemons[0]) return message.channel.send("You need to pick a starter pokemon using the \`" + prefix.toLowerCase() + "start\` command before using this command!");
        ...
        if (!user2 || !user2.pokemons[0]) return message.channel.send(user1.user.username + " need to pick a starter pokemon using the \`" + prefix.toLowerCase() + "start\` command before using this command!");
;
                    ...
                    if (PokemonSpeed > pokemonTwoSpeed) {
                      ...
                      let damage = Math.floor(Math.random() * 20) + 5;
                        ...
                        if (hp < 1) {
                            ...
                            embed
                                .setTitle (`Battle ended!`)
                                .setDescription(`<@${message.author.id}>'s ${user.pokemons[selected].name.capitalize()} VS <@${user1.id}>'s ${user2.pokemons[selected1].name.capitalize()}\n\nBattle ended and ${user1} won the Battle and was rewarded with 10  :moneybag: Coins`)
                                .attachFiles([{ name: "Battle.png", attachment: canvas.toBuffer() }])
                                .setImage("attachment://" + "Battle.png")
                                .setColor(color)
                            return em.edit(embed)
