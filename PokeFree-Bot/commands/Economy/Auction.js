...
module.exports = {
    ...
    if(!abc.includes(message.guild.id)) return message.channel.send(` > Auctions can only be held in the Official Server`);
        ...
        if (message.content.includes(guild.prefix + "auction listings") || message.content.includes(guild.prefix + "a listings") || message.content.includes(guild.prefix + "a ls") || message.content.includes(guild.prefix + "auction ls") || message.content.includes(guild.prefix + `auction ${Number(args[0])} listings`) || message.content.includes(guild.prefix + `auction ${Number(args[0])} ls`) || message.content.includes(guild.prefix + `a ${Number(args[0])} listings`) || message.content.includes(guild.prefix + `a ${Number(args[0])} ls`)) {
            ...
            if (all[0]) {
                ...
                if (all[20] && message.content.endsWith(`${guild.prefix}a lt`) || all[20] && message.content.endsWith(`${guild.prefix}auction lt`) || all[20] && message.content.endsWith(`${guild.prefix}auction listings`) || all[20] && message.content.endsWith(`${guild.prefix}a listings`)) {
                    ...
                    embed
                        ...
                        .setColor('#add8e6')
                        .setDescription(chunks[0].map((r, i) => `**Level ${r.pokemon.level} ${(r.pokemon.shiny ? "✨" : "")}${r.pokemon.name.replace(/-+/g, " ").replace(/\b\w/g, l => l.toUpperCase())}** | ID: ${r.num} | IV: ${r.pokemon.totalIV}% | Bid: ${new Intl.NumberFormat('en-IN').format(r.bid)} Psycoin(s) `).join('\n') || "There are No Item Listed in the Auctions!")
          ...
          // are bhai bhai ye kya bna diyo
            ...
            if (zbc["shiny"] || zbc["sh"]) s = s.filter(e => { if (e.pokemon.shiny) return e });
            ...
            if (isNaN(args[1])) return message.channel.send(`Invalid Pokémon number provided. It should be in the form of \`${prefix}auction list <pokemon number> <auction_timeout> [buyout]\``);
            let num = parseInt(args[1]) - 1;
          ...
          // check if author has lower balance or not bruh!
            let auction = await Auction.find({});
            let bid = parseInt(args[2]);
            let user1 = await User.findOne({id: message.author.id})
            if (user1.balance < bid) return message.channel.send(`You don't have enough balance nab!`)
