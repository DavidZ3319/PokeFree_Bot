...
module.exports = {
  ...
  let e = message,
    ...
    if (zbc["legendary"] || zbc["legend"]|| zbc["leg"]) s = s.filter(e => {
    ...
    if (zbc["mythical"] || zbc["mythic"]||zbc["my"]) s = s.filter(e => {
    ...
    if (isNaN(e[0])) txt = s.map((item, i) => `\`${item.num}\` ${item.shiny ? "✨ " : ""} • **${item.name.replace(/-+/g, " ").replace(/\b\w/g, l => l.toUpperCase())}** ${item.fav ? ":heart: " : ""} • Lvl ${item.level} •  ${item.totalIV}%${(item.nick != null ? ` • Nickname: ${item.nick}` : "")}`).slice(0, 15).join("\n")
    if (Number(args[0])) {
      ...
      embed
        ...
        .setColor('#36393E')
        .setDescription((chunks[index].map((item, i) => { return `\`${item.num}\` ${item.shiny ? "✨ " : ""} • **${item.name.replace(/-+/g, " ").replace(/\b\w/g, l => l.toUpperCase())}** ${item.fav ? ":heart: " : ""} | Lvl ${item.level} •  ${item.totalIV}%${(item.nick != null ? ` • Nickname: ${item.nick} ` : "")}` }).join("\n")))
