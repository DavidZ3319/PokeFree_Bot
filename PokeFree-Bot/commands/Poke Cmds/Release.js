...
module.exports = {
  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
    ...
    if (args[0].toLowerCase() == "latest" || args[0].toLowerCase() == "l" || args[0].toLowerCase() == "0") {
      ...
      if (num == undefined) return message.channel.send(`\`\`\`\nError: No <p_Id> recevied / Must sellect 5 pokemons to release - ${module.exports.usage[0]}\n\`\`\``)
      ...
      collector.on('collect', async (reaction, userx) => {
        if (reaction.emoji.name === "✅") {
          ...
          return message.channel.send(`Successfully released the pokemons and you also received 2 Pokecoins `)
